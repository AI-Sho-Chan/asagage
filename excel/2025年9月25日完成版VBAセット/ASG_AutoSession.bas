Attribute VB_Name = "ASG_AutoSession"
Option Explicit

Public Sub Open3Min_RefreshAndExport()
    ...
    Do While Timer - t0 < 180 '3分
        Application.CalculateFull
        Call Recalc_Signal_And_Final
        DoEvents
        Application.Wait Now + TimeSerial(0, 0, 5) '5秒間隔
    Loop

    '抽出 & 並べ替え
    OUT.Cells.Clear
    OUT.Range("A1:F1").Value = Array("コード", "銘柄名", "方向", "O(円/株)", "P(株)", "期待値=O*P")
    Dim last&, r&, rowOut&
    ...
    Next
    If rowOut > 2 Then
        OUT.Range("A2:F" & rowOut - 1).Sort Key1:=OUT.Range("F2"), Order1:=xlDescending, Header:=xlNo
    End If
    MsgBox "3分セッション終了。Signals に期待値降順で出力。", vbInformation
End Sub

Private Sub Recalc_Signal_And_Final()
    Dim D As Worksheet: Set D = Worksheets("Dashboard")
    Dim s As Worksheet: Set s = Worksheets("Settings")
...
' 依存ヘルパ（前モジュールと同一実装でOK。重複定義を避けるなら共通化）
